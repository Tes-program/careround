...
class PatientDeteriorationConsumerTest {

    @Mock private NotificationService notificationService;
    @Mock private CoreLookupClient coreLookupClient;
    @Mock private NotificationIdempotencyGuard idempotencyGuard;

    private PatientDeteriorationConsumer consumer;

    @BeforeEach
    void setUp() {
        consumer = new PatientDeteriorationConsumer(new ObjectMapper(), notificationService, coreLookupClient, idempotencyGuard);
    }

    @Test
    void validPayload_createsTwoNotifications() {
        when(coreLookupClient.findWardSupervisorId("ward-1")).thenReturn(Optional.of("super-1"));

        consumer.listen(payload("doc-1"));
        ...
        assertThat(captor.getAllValues()).extracting(Notification::getRecipientId)
                .containsExactly("doc-1", "super-1");
    }

    @Test
    void assignedToIdNull_createsOnlyOneSupervisorNotification() {
        when(coreLookupClient.findWardSupervisorId("ward-1")).thenReturn(Optional.of("super-1"));
    ...
    private String payload(String assignedToId) {
        return """
                {"hospitalId":"hosp-1","patientId":"patient-1","wardId":"ward-1","newsScore":7,"severity":"HIGH","escalationId":"esc-1","assignedToId":%s,"correlationId":"corr-1"}
