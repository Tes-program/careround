...
class ClinicalNoteControllerTest {
    ...
    void setUp() {
        ...
        sampleNote = new ClinicalNoteResponse(
                "note-1", "patient-1", "hosp-1", "user-1",
                NoteType.WARD_ROUND_NOTE, "Patient is stable.", null,
                false, null, null,
    ...
    @Test
    void createNote_asDoctor_returns201() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/clinical-notes")
                        ...
                        .content(objectMapper.writeValueAsString(
                                new CreateClinicalNoteRequest("patient-1", NoteType.WARD_ROUND_NOTE,
                                        "Patient is stable.", null, false, null))))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.data.noteType").value("WARD_ROUND_NOTE"));
    }

    @Test
    void createNote_missingContent_returns400() throws Exception {
        String invalid = """
                {"patientId":"patient-1","noteType":"WARD_ROUND_NOTE"}
                """;
